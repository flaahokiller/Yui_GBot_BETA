from Yui_GBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Yui_GBot import SUDO_USERS
...
async def Yui_GBot_is_admin(chat_id: int):
    status = False
    Yui_GBot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Yui_GBot.id == user.id:
            status = True
            break
    return status
