...
import Yui_GBot.modules.sql.feds_sql as sql
...
from Yui_GBot.modules.helper_funcs.alternate import send_message
...
from Yui_GBot.modules.helper_funcs.string_handling import markdown_parser
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in OP_USERS:
        message.reply_text("OP_USERS cannot be fed banned!")
        return

    if int(user_id) in SAFE_USERS:
        message.reply_text("SAFE_USERS cannot be fed banned!")
        return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Yui_GBot_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Yui_GBot_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Yui_GBot_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Yui_GBot_fbanned_users.csv",
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if int(import_userid) in SUDO_USERS:
                        failed += 1
                        continue
                    if int(import_userid) in OP_USERS:
                        failed += 1
                        continue
                    if int(import_userid) in SAFE_USERS:
                        failed += 1
                        continue
