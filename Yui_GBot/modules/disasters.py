...
from Yui_GBot.modules.helper_funcs.chat_status import (
    dev_plus,
    sudo_plus,
    whitelist_plus,
)
from Yui_GBot.modules.helper_funcs.extraction import extract_user
from Yui_GBot.modules.log_channel import gloggable
from telegram import ParseMode, TelegramError, Update
from telegram.ext import CallbackContext, CommandHandler, run_async
from telegram.utils.helpers import mention_html

ELEVATED_USERS_FILE = os.path.join(os.getcwd(), "Yui_GBot/elevated_users.json")
...
def addtiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in OP_USERS:
        message.reply_text("This user is already a Tiger.")
        return ""

    data["OP_USERS"].append(user_id)
    OP_USERS.append(user_id)
...
def removetiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in OP_USERS:
        message.reply_text("Demoting to normal user")
        OP_USERS.remove(user_id)
        data["OP_USERS"].remove(user_id)
...
Group admins/group owners do not need these commands. 
 ...
 ╠ `/SUDO_USERS`*:* Lists all Dragon disasters
 ╠ `/SUPPORT_USERS`*:* Lists all Demon disasters
 ╠ `/OP_USERS`*:* Lists all OP_USERS disasters
 ╠ `/SAFE_USERS`*:* Lists all Wolf disasters
...
WHITELISTLIST_HANDLER = CommandHandler(["whitelistlist", "SAFE_USERS"], whitelistlist)
TIGERLIST_HANDLER = CommandHandler(["OP_USERS"], tigerlist)
SUPPORTLIST_HANDLER = CommandHandler(["supportlist", "SUPPORT_USERS"], supportlist)
SUDOLIST_HANDLER = CommandHandler(["sudolist", "SUDO_USERS"], sudolist)
