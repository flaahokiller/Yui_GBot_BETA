...
from Yui_GBot.__main__ import STATS, TOKEN, USER_INFO
import Yui_GBot.modules.sql.userinfo_sql as sql
from Yui_GBot.modules.disable import DisableAbleCommandHandler
from Yui_GBot.modules.sql.global_bans_sql import is_user_gbanned
from Yui_GBot.modules.sql.afk_sql import is_afk, check_afk_status
from Yui_GBot.modules.sql.users_sql import get_user_num_chats
from Yui_GBot.modules.helper_funcs.chat_status import sudo_plus
from Yui_GBot.modules.helper_funcs.extraction import extract_user
from Yui_GBot import telethn as Yui_GBotTelethonClient, OP_USERS, SUDO_USERS, SUPPORT_USERS
...
@Yui_GBotTelethonClient.on(
    events.NewMessage(
        pattern="/ginfo ", from_users=(OP_USERS or []) + (SUDO_USERS or []) + (SUPPORT_USERS or [])
    )
)
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        ...
        disaster_level_present = True
    elif user.id in OP_USERS:
        text += "\n\nThe Disaster level of this person is 'Tiger'."
        disaster_level_present = True
    elif user.id in SAFE_USERS:
