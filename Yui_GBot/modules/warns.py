...
import telegram
from Yui_GBot import OP_USERS, SAFE_USERS, dispatcher
...
from Yui_GBot.modules.helper_funcs.extraction import (
    extract_text,
    extract_user,
    extract_user_and_text,
)
from Yui_GBot.modules.helper_funcs.filters import CustomFilters
from Yui_GBot.modules.helper_funcs.misc import split_message
from Yui_GBot.modules.helper_funcs.string_handling import split_quotes
from Yui_GBot.modules.log_channel import loggable
from Yui_GBot.modules.sql import warns_sql as sql
...
from Yui_GBot.modules.sql.approve_sql import is_approved
...
def warn(
    ...
    if user.id in OP_USERS:
        if warner:
            message.reply_text("OP_USERS cant be warned.")
        else:
            message.reply_text(
                "Tiger triggered an auto warn filter!\n I can't warn OP_USERS but they should avoid abusing this."
            )
        return
    ...
    if user.id in SAFE_USERS:
        if warner:
            ...
            message.reply_text(
                "Wolf Disaster triggered an auto warn filter!\nI can't warn SAFE_USERS but they should avoid abusing this."
            )
        return
    ...
    if num_warns >= limit:
        ...
        # message.bot.send_sticker(chat.id, BAN_STICKER)  # Yui_GBot's sticker
        keyboard = None
        log_reason = (
